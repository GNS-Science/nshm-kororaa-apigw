...
function buildTransforms(schemaName, settings) {
  ...
  const hazardResolvers = ['KORORAA_gridded_location', 'KORORAA_hazard_curves', 'KORORAA_gridded_hazard' ]
  transforms = transforms.concat([
    new FilterRootFields((operation, fieldName) => {
      const exclude = (schemaName === 'kororaa') & (operation === 'Query') & (hazardResolvers.includes(fieldName) === true) ;
