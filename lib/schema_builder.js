...
function buildTransforms(schemaName, settings) {
  ...
  let hazard_resolvers = ['KORORAA_gridded_location', 'KORORAA_hazard_curves', 'KORORAA_gridded_hazard' ]
  transforms = transforms.concat([
    new FilterRootFields((operation, fieldName) => {
      const exclude = (schemaName=='kororaa') & (operation === 'Query') & (hazard_resolvers.includes(fieldName) === true) ;
      if (hazard_resolvers.includes(fieldName) === true) {
        logger.debug(
