...
describe('toshi mocked store', () => {
  ...
  test('mocked object returns id', async () => {
     expect(store.get("ScaledInversionSolution", 'U2NhbGVkSW52ZXJzaW9uU29sdXRpb246MTE4NTQ2', 'id')).toEqual('U2NhbGVkSW52ZXJzaW9uU29sdXRpb246MTE4NTQ2')
   })

  test('mocked object returns valid typename', async () => {
     expect(store.get("ScaledInversionSolution", 'U2NhbGVkSW52ZXJzaW9uU29sdXRpb246MTE4NTQ2')).toEqual(
      {"$ref": {"key": "U2NhbGVkSW52ZXJzaW9uU29sdXRpb246MTE4NTQ2", "typeName": "ScaledInversionSolution"}}
      )
   })

  test('returns ref from object', async () => {
     const nodeRef = {$ref: {key: 'U2NhbGVkSW52ZXJzaW9uU29sdXRpb246MTE4NTQ2', typeName: 'ScaledInversionSolution'}}
     expect(store.get("ScaledInversionSolution", {id:'U2NhbGVkSW52ZXJzaW9uU29sdXRpb246MTE4NTQ2'})).toEqual(nodeRef)
   })

  test('knows the schema types', async () => {
     expect(() => store.get("HUH", 'U2NhbGVkSW52ZXJzaW9uU29sdXRpb246MTE4NTQ2', 'id')).toThrow('HUH does not exist on schema or is not an object or interface')
   })
  ...
  test('mocked store returns mocked Node object', async () => {
    expect(store.get("Node", 'U2NhbGVkSW52ZXJzaW9uU29sdXRpb246MTE4NTQ2', 'id')).toEqual("U2NhbGVkSW52ZXJzaW9uU29sdXRpb246MTE4NTQ2")
    expect(store.get("ScaledInversionSolution", 'U2NhbGVkSW52ZXJzaW9uU29sdXRpb246MTE4NTQ2', 'id')).toEqual("U2NhbGVkSW52ZXJzaW9uU29sdXRpb246MTE4NTQ2")
    expect(store.get("ScaledInversionSolution", 'U2NhbGVkSW52ZXJzaW9uU29sdXRpb246MTE4NTQ2', 'file_name')).toEqual('abc.zip')
  })
...
describe('toshi with custom mocks', () => {
    ...
    resolvers: store => {
      ...
      return {
        QueryRoot: {
          ...
          nodes: () => ({SearchResultConnection: { edges: [{ SearchResultEdge: {
            node: {
              typeName: 'ScaledInversionSolution',
              key: 'U2NhbGVkSW52ZXJzaW9uU29sdXRpb246MTE4NTQW',
              fieldName: 'file_name',
  ...
  test.skip('mocked store via query with our Node object', async () => {
    ...
    expect(errors).not.toBeDefined();
    expect(data).toBeDefined();
    expect(data.nodes.result.edges[0].node).toEqual('Goodness me');
  })
